-- =====================================================
-- BASE DE DATOS COMPLETA UNIFICADA - SONRISITAPP
-- =====================================================
...
-- Insertar turnos de ejemplo (fechas actualizadas para hoy y próximos días)
...
UNION ALL
SELECT 'Odontólogos', COUNT(*) FROM odontologos  
UNION ALL
...
-- Mostrar estadísticas para el landing page
SELECT 
    (SELECT COUNT(*) FROM usuarios WHERE rol != 'admin') as usuarios_registrados,
    (SELECT COUNT(*) FROM turnos WHERE estado IN ('reservado', 'confirmado', 'completado')) as turnos_activos,
    (SELECT COUNT(*) FROM turnos WHERE fecha = CURDATE() AND estado IN ('reservado', 'confirmado')) as turnos_hoy,
    (SELECT COUNT(*) FROM turnos WHERE estado = 'completado') as turnos_completados,
    (SELECT COUNT(*) FROM odontologos WHERE activo = 1) as odontologos_activos,
    (SELECT COUNT(*) FROM servicios WHERE activo = 1) as servicios_disponibles;

SELECT '✅ Base de datos creada exitosamente' as mensaje;ROM turnos) as turnos_totales,
    (SELECT COUNT(*) FROM odontologos WHERE activo = TRUE) as odontologos_activos,
    (SELECT COUNT(*) FROM servicios WHERE activo = TRUE) as servicios_disponibles;ROM turnos WHERE estado IN ('reservado', 'confirmado', 'completado')) as turnos_activos,
    (SELECT COUNT(*) FROM turnos WHERE fecha = CURDATE() AND estado IN ('reservado', 'confirmado')) as turnos_hoy,
    (SELECT COUNT(*) FROM turnos WHERE estado = 'completado') as turnos_completados,
    (SELECT COUNT(*) FROM odontologos WHERE activo = 1) as odontologos_activos,
    (SELECT COUNT(*) FROM servicios WHERE activo = 1) as servicios_disponibles;
